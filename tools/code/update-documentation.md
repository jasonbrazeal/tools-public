...
- `trace_log`: string or null — for any `drift`/`stale_reference`/`ambiguous`/`unresolved_stale`, a one-line record of the three resolution traces run (repo-search hits, entry-point leaf, succession commit) and what each returned; null only for `consistent` claims that verified on the anchor file alone
...
- **Human-review:** `status` is `ambiguous`, `unresolved_stale`, `unverifiable`, or `command_unrunnable`; OR `drift`/`stale_reference` with `confidence` of `medium` or `low`; OR any claim where the prose is newer than the code; OR any `self_contradiction`. These are never edited. Each `ambiguous`/`unresolved_stale` finding must show the `trace_log` proving tracing was exhausted — an escalation without a trace is itself a defect to be corrected before the report ships.
